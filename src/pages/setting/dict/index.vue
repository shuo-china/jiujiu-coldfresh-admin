<template>
  <div class="flex h-full gap-x-6 p-6">
    <el-card
      ...
      body-class="flex flex-wrap gap-6 content-start"
      shadow="never"
    >
      <div class="min-h-25 max-w-full">
        <el-button
          ...
          icon="Plus"
          plain
          class="h-full w-85 max-w-full border-dashed"
      ...
      <el-card
        ...
        :class="
          activeDictTypeId === item.id ? 'border-primary bg-primary/10' : ''
        "
        shadow="hover"
        header-class="p-4"
        body-class="p-4"
        @click="activeDictTypeId = item.id"
      >
        <template #header>
    ...
    />
    <dict-item-form
      :type-id="activeDictTypeId"
      :mode="dictItemMode"
      v-model:visible="dictItemFormVisible"
      :id="dictItemId"
      @finished="refreshDictItemList"
    />
  </div>
</template>

<script setup lang="ts">
import { getDictTypeListApi, deleteDictTypeApi } from "@/api/dict_type";
import { getDictItemListApi, deleteDictItemApi } from "@/api/dict_item";
import useRequest from "@/hooks/useRequest";
import DictTypeForm from "./DictTypeForm.vue";
import DictItemForm from "./DictItemForm.vue";
import { ElMessageBox } from "element-plus";

const { data: dictTypeList, refresh: refreshDictTypeList } =
  useRequest(getDictTypeListApi);
const {
  run,
  data: dictItemList,
